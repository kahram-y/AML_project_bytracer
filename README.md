...
├── ml/
│   ├── feature_engineering_v2.ipynb     # V2 행동 통계 피처
│   ├── feature_engineering_v3.ipynb     # V3 그래프 피처
│   └── ensemble.ipynb                   # V2+V3 Max 앙상블
│
├── gnn/
│   ├── tgat_v7c18.py                    # TransformerConv 기반 TGAT
│   └── tgat_v7c19.py                    # 레이어 비교 실험 (4종 10config)
│
├── graphsage/
│   └── graphsage_adan.py                # GraphSAGE + ADAN 레이어
│
└── report/
    └── 최종보고서.pdf
```
...
```bash
# 1. 환경 설치
pip install polars torch torch_geometric xgboost scikit-learn tqdm psutil

# 2. ML 앙상블 실행
jupyter notebook ml/ensemble.ipynb

# 3. TGAT (TransformerConv) 실행
python gnn/tgat_v7c18.py

# 4. 레이어 비교 실험 (NNConv+MAX 포함)
python gnn/tgat_v7c19.py
```
